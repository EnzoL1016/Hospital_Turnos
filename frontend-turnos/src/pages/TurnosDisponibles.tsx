...
const TurnosDisponibles = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto mt-10 p-6 bg-white shadow-lg rounded-lg">
      <h2 className="text-2xl font-bold mb-6 text-blue-700 border-b pb-3">Reservar Turno</h2>
      ...
      <div className="space-y-4 mb-6">
        <div>
          ...
          >
            <option value="">
              {isLoadingProfesionales ? "Cargando..." : "Seleccione una especialidad"}
            </option>
        ...
        <div>
          ...
          >
            ...
            {profesionales
              .filter((p) => p.especialidad === selectedEspecialidad)
              .map((p) => (
      ...
      {turnosQuery.isError && (
        <p className="text-red-500 text-center">No se pudieron cargar los turnos.</p>
      )}
        ...
        <>
          {turnos.length > 0 ? (
            <>
                  ...
                  >
                    <div>
                      <p className="text-sm font-semibold text-gray-800">
                        {formatDateFromYYYYMMDD(turno.fecha)}
                      </p>
                    ...
                    <button
                      ...
                      disabled={reservarTurnoMutation.isPending}
              ...
              {/* Desktop: tabla */}
              <table className="w-full border border-gray-300 mt-6 rounded-lg overflow-hidden hidden md:table">
                <thead>
              ...
              <div className="mt-6">
                <div className="flex overflow-x-auto gap-2 px-1 py-2 md:justify-center scrollbar-hide">
                  <button
                    disabled={page === 1}
                    onClick={() => setPage(page - 1)}
                    className="min-w-[80px] px-3 py-2 rounded bg-gray-200 hover:bg-gray-300 disabled:opacity-50 text-sm"
                  >
                    Anterior
                  </button>
                  {[...Array(totalPages)].map((_, i) => (
                    <button
                      key={i + 1}
                      onClick={() => setPage(i + 1)}
                      className={`min-w-[40px] px-3 py-2 rounded text-sm ${
                        page === i + 1
      ...
      {turnoParaReservar && (
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p>
                ¿Deseas reservar el turno para el día{" "}
                <strong className="font-semibold">
                  {formatDateFromYYYYMMDD(turnoParaReservar.fecha)}
                </strong>{" "}
                a las{" "}
                <strong className="font-semibold">
                  {turnoParaReservar.hora_inicio.slice(0, 5)}
                </strong>
                ?
              </p>
            </div>
